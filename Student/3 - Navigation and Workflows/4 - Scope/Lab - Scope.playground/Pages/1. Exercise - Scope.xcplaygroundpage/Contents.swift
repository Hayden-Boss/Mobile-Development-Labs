...
//"foo" is a local variable, meaning its only usable in this specific for loop
...
//x is first created globally, so it can be used outside of the local scope. foo is defined in the local scope so it can only be used in said scope.
